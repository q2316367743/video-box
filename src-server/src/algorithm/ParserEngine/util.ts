 ...
 * @param elements
 */
export function parseCoreElements(select: string, elements: Array<CheerioAPI>): Array<CheerioAPI> {
  if (!elements || elements.length === 0) {
    return new Array<CheerioAPI>();
  }
 ...
 * @param elements
 */
export function parseIndexElements(select: string, elements: Array<CheerioAPI>): Array<CheerioAPI> {
  if (!elements || elements.length === 0) {
    return new Array<CheerioAPI>();
  }
  ...
  const items = new Array<CheerioAPI>();
 ...
 * @param elements 元素
 */
function parseSplitElements(select: string, elements: Array<CheerioAPI>): ParseResult {
...
function parseSplitElementsToString(select: string, elements: Array<CheerioAPI>): string {
 ...
 * @param elements 元素
 */
function parseSplitElementsToElement(select: string, elements: Array<CheerioAPI>): Array<CheerioAPI> {
  // 默认选择器
  let strings = select.split("@");
...
interface ParseConnectorElementsOption<R> {
  parseSplitElements: (select: string, elements: Array<CheerioAPI>) => R,
  defaultValue: R,
...
export function parseConnectorElements<R>(
  select: string, elements: Array<CheerioAPI>, option: ParseConnectorElementsOption<R>): R {
  if (!elements || elements.length === 0) {
 ...
 * @param elements
 */
export function parseConnectorElementsToString(select: string, elements: Array<CheerioAPI>): string {
...
function turnElements(items: Array<Array<CheerioAPI>>): Array<CheerioAPI> {
  const max = Math.max(...items.map(e => e.length));
  const res = new Array<CheerioAPI>();
  for (let i = 0; i < max; i++) {
 ...
 * @param elements
 */
export function parseConnectorElementsToElements(select: string, elements: Array<CheerioAPI>): Array<CheerioAPI> {
  return parseConnectorElements<Array<CheerioAPI>>(select, elements, {
    defaultValue: [],
 ...
 * @param elements
 */
export function parseXpathElementsToString(select: string, elements: Array<CheerioAPI>): string {
  return parseConnectorElements<string>(select, elements, {
    defaultValue: '',
    parseSplitElements: (select: string, elements: Array<CheerioAPI>) => {
 ...
 * @param elements
 */
export function parseXpathElementsToElements(select: string, elements: Array<CheerioAPI>): Array<CheerioAPI> {
  return parseConnectorElements<Array<CheerioAPI>>(select, elements, {
    defaultValue: [],
    parseSplitElements: (select: string, elements: Array<CheerioAPI>) => {
      return new Array<CheerioAPI>();
...
export function parseLastNodeToString(result: ParseResult): string {
  ...
  if (attr === 'text') {
    text = elements.map(element => element.text() || '').join(join);
  } else if (last === 'textNodes') {
    const lines = new Array<string>();
    for (let element of elements) {
      for (let child of element("*").children()) {
        const textContent = load(child).text();
        if (textContent) {
    ...
    text = elements.map(element => element.html() || '').join(join);
    ...
    try {
      text = elements.map(element => element("*").attr(last) || '').join(join);
    } catch (e) {
      console.error(e);
    }
  }
