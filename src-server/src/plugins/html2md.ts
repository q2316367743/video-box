 ...
 */
function html2md(htmlData: string) {
  let codeContent: Array<string> | null;  // code标签数据
  let preContent: Array<string> | null;  // pre标签数据
  let tableContent: Array<string> | null; // table标签数据
  let olContent: Array<string> | null; // ol标签数据
  let imgContent: Array<string> | null;  // img标签数据
  let aContent: Array<string> | null; // a标签数据
  ...
  function olRecover(olData = '') {
    let result = olData
    let num = olData.match(/<li>/ig)?.length || 0
    for (let i = 1; i <= num; i++) {
      let line = '[~wrap]'
  ...
  // 函数：复原table标签
  function tableRecover(tableData: Array<Array<string> | null> | null = null) {
    if (!tableData) return '';
    if (tableData[0] == null) {  // 如果不存在 th 标签，则默认表格为一层
      let result = ''
      let colNum = tableData[1]?.length || 0
    ...
    result += `|[~wrap]`
    for (let k = 0; k < (tableData[1]?.length || 0);) {
  ...
  let tbodyContent = new Array<Array<Array<string> | null>>();
  if (tableContent !== null) {  // 分析储存
    for (let i = 0; i < tableContent.length; i++) {
      tbodyContent[i] = new Array<Array<string> | null>();  // tbodyContent[i]的第一个数据是thead数据,第二个是tbody的数据
  ...
  if (preContent !== null) {
    for (let k = 0; k < preContent.length; k++) {
      ...
      let preLanguageS = (preLanguage != null && preLanguage[0] != 'undefined') ? preLanguage[0] + '\n' : '\n'
      pureHtml = pureHtml.replace(/`#preContent#`/i, preLanguageS + preText)
    }
  }
