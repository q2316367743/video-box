...
import type {WebItemView} from "@/pages/web/pages/list/types/WebItem.ts";
import MessageBoxUtil from "@/utils/modal/MessageBoxUtil.tsx";
import MessageUtil from "@/utils/modal/MessageUtil.ts";
import {DialogPlugin, Paragraph, Select} from "tdesign-vue-next";

function moveFileToFolder(source: WebItemView) {
  const folder = ref('');
  const options = computed(() => [{
    label: '根目录',
    value: '',
  }, ...useWebFolderStore().webFolders.map(f => ({
    label: f.name,
    value: f.id,
  }))])
  const p = DialogPlugin({
    ...
    default: () => <div>
      ...
      <Select v-model={folder.value} options={options.value} autofocus={true} clearable={true} filterable={true}/>
    </div>,
    onConfirm: () => {
      useVideoSourceStore().update({id: source.id, folder: folder.value})
        .then(() => {
...
export const handleListContextmenu = (e: MouseEvent) => {
...
export const handleItemContextmenu = (e: MouseEvent, source: WebItemView, openInfo: (source: WebItemView) => void) => {
  CtxMenu.showContextMenu({
    x: e.x,
    y: e.y,
    theme: isDark.value ? 'dark' : 'light',
    items: source.type === 'file' ? [
      {
        label: '打开',
        icon: () => <CircleIcon/>,
      ...
      },
      {
        label: '修改',
        icon: () => <EditIcon/>,
        onClick: () => openVideoSourceDialog(source.source)
      },
      {
        label: '移动到',
        icon: () => <ChevronRightIcon/>,
        onClick: () => moveFileToFolder(source)
      },
      {
